...
const register = 'faro-register.js?v=3';
const home = 'faro-home-r2.js?v=2';
const finance = 'faro-finance.js?v=1';
const polish = 'faro-r2-polish.js?v=2';

assert.match(shell, /faro-home-r2\.js\?v=2/, 'shell precisa carregar Home premium');
assert.match(shell, /faro-register-earnings\.js\?v=1/, 'shell precisa carregar helper de ganhos por origem');
assert.match(shell, /faro-register\.js\?v=3/, 'shell precisa carregar Registrar premium');
assert.match(shell, /faro-r2-polish\.js\?v=2/, 'shell precisa carregar Histórico/Central premium');
...
assert.ok(shell.indexOf(polish) > shell.indexOf(finance), 'Polish deve continuar depois dos owners operacionais');

assert.match(sw, /const CORE_CACHE = 'faro-v1-core-23'/, 'batch premium precisa renovar o core cache instalado');
...
console.log('platform-earnings-delivery-contract: Home, Registrar, Polish, PWA e assets na mesma geração — ok');
