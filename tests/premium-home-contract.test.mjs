...
assert.match(home,/registerButton\?\.classList\.add\('faro-action-card'\)/,'Registrar precisa assumir Action Card quando presente');
assert.match(home,/weekCard\?\.classList\.add\('faro-state-card'\)/,'Semana precisa assumir State Card quando presente');
assert.match(home,/monthCard\?\.classList\.add\('faro-action-card'\)/,'Mês precisa assumir Action Card quando presente');
