...
assert.match(shell, /faro-planning\.js\?v=1/);
...
for (const source of [sw, await read('_site/sw.js')]) {
  assert.match(source, /faro-v1-core-13/);
  ...
  assert.match(source, /faro-planning\.js\?v=1/);
...
for (const path of ['faro-brand-r2.js','faro-home-r2.js','faro-planning.js','faro-planning-invariants.js','faro-tour.js','faro-platform.js','faro-update.js','faro-state.js','faro-energy.js','faro-onboarding.js','sw.js','app-shell.html']) {
...
console.log('FARO UX-R2: núcleo financeiro e shell legado intactos; nova experiência está integralmente no build — ok');
