...
const builtUpdate = await read('_site/faro-update.js');
...
assert.equal(builtUpdate, update);
...
for (const path of ['faro-platform.js','faro-update.js','faro-state.js','faro-energy.js','faro-home.js','faro-onboarding.js','sw.js','app-shell.html']) {
  assert.ok((await stat(path)).size > 0, `${path} não pode estar vazio`);
}

console.log('FARO: onboarding retomável, atualização silenciosa, estado moderno preservado, elétrico coerente, PWA protegido e núcleo financeiro intacto — ok');