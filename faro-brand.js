(() => {
  ...
  const setupDailyJourney = () => {
    ...
    if (!dashboard || !planning || !dayView || document.getElementById('faroDailyResult')) return;

    if (!document.getElementById('faroDailyStyles')) {
      const style = document.createElement('style');
      style.id = 'faroDailyStyles';
      style.textContent = `
        #faroPlanningDetails{display:grid;gap:1.5rem}
        #faroPlanningDetails>.card-vetta{margin:0}
    ...
    replaceExactText(dashboard, 'Ver planejamento do mês', 'Ver detalhes da meta');
    ...
    const planningDetails = document.createElement('div');
    planningDetails.id = 'faroPlanningDetails';
    planningDetails.innerHTML = '<div><span class="label-micro !text-blue-600">Detalhes da meta</span><h3 class="font-extrabold text-lg">Ajuste e entenda o plano quando precisar</h3><p class="text-xs text-slate-500 mt-1">Esses controles saíram do Início para deixar o uso diário mais rápido.</p></div>';
    planning.appendChild(planningDetails);
